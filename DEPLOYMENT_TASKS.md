...
- [ ] **Push Code**: Run the following commands in your `mern-login` folder terminal:
  ...
  # Executed by agent
  git branch -M main
  git remote add origin https://github.com/gokul2006-gs/mern-fleet-app.git
  git push -u origin main
  # (Credentials may be requested via a popup)
  ```
- [x] **Push Code**: Code successfully pushed to GitHub! (Verified)

## 3. Database (MongoDB Atlas)
- [x] **Network Access**: Go to MongoDB Atlas -> Network Access.
- [x] **Allow All IPs**: Add IP Address `0.0.0.0/0` (In "Allow Access from Anywhere"). This allows Render/Vercel to connect. (Note: For stricter security, this can be refined later, but this is standard for serverless hosts).
...
- [x] **Deploy**: Click "Create Web Service". Wait for it to show "Live".
- [x] **Copy URL**: `https://mern-fleet-app.onrender.com` (Live!)
...
- [x] **Environment Variables**:
    - Add `VITE_API_URL` with value: `https://mern-fleet-app.onrender.com` (The URL you copied from Step 4). Do not add a trailing slash.
    - Add `VITE_GOOGLE_CLIENT_ID`: (Copy from `frontend/.env`).
- [x] **Deploy**: Click "Deploy".
- [x] **Success**: Live at `https://mern-fleet-app.vercel.app`

## 6. Final Test
- [ ] **Login/Register**: Try to register a new user on the deployed site.
- [ ] **Check Auth**: Ensure Google Login works (Update Google Cloud Console).
